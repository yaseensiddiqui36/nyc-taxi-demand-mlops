    ...
    def _run_backfill(**ctx) -> None:
        ...
        with get_session() as session:
            existing = set(
                session.execute(
                    text(
                        "SELECT DISTINCT DATE_TRUNC('month', pickup_hour)::DATE FROM hourly_rides"
                    )
                ).scalars()
            )
        ...
        to_load = [
            (y, m) for y, m in months if datetime(y, m, 1).date() not in existing
        ]
        ...
        for i, (year, month) in enumerate(to_load, 1):
            ...
            try:
                ...
                if not report.passed:
                    print(
                        f"  WARNING: drop_rate={report.drop_rate:.1%}, loading anyway"
                    )
