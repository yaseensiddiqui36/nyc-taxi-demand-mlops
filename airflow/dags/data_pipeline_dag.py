...
with DAG(
    ...
    def _check_new_data(**ctx) -> str:
        ...
        if result and result > 0:
            print(
                f"Data for {year}-{month:02d} already loaded ({result:,} rows). Skipping."
            )
            return "no_new_data"
    ...
    def _fetch_and_validate(**ctx) -> None:
        ...
        if not report.passed:
            alert_validation_failure(
                year, month, report.drop_rate, report.failure_reasons
            )
            raise ValueError(
        ...
        ti.xcom_push(
            key="clean_df_path", value=f"/tmp/clean_{year}_{month:02d}.parquet"
        )
    ...
    def _push_to_feast_offline(**ctx) -> None:
        ...
        print(
            "Offline store is PostgreSQL (public.hourly_rides) — skipping push, proceeding to materialize."
        )
    ...
    def _run_drift_check(**ctx) -> None:
        ...
        if len(current) > 1000 and len(reference) > 1000:
            ...
            print(
                f"Drift report generated. Current rows: {len(current)}, Ref rows: {len(reference)}"
            )
        else:
    ...
    def _notify_success(**ctx) -> None:
        ...
        alert_pipeline_success(
            "data_pipeline_dag", f"Loaded data for {year}-{month:02d}"
        )

    # ── Task definitions ──────────────────────────────────────

    check = BranchPythonOperator(
        task_id="check_new_data", python_callable=_check_new_data
    )
    no_new = EmptyOperator(task_id="no_new_data")
    fetch = PythonOperator(
        task_id="fetch_and_validate", python_callable=_fetch_and_validate
    )
    transform = PythonOperator(
        task_id="transform_to_timeseries", python_callable=_transform_to_timeseries
    )
    load_pg = PythonOperator(
        task_id="load_to_postgres", python_callable=_load_to_postgres
    )
    feast_off = PythonOperator(
        task_id="push_to_feast_offline", python_callable=_push_to_feast_offline
    )
    feast_on = PythonOperator(
        task_id="materialize_feast_online", python_callable=_materialize_feast_online
    )
